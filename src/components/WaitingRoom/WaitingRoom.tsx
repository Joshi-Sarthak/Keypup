...
function WaitingRoom({ id, name, email }: { id: string; name: string; email: string }) {
    ...
    >
        ...
        <span className={`relative text-sm md:text-base font-semibold uppercase tracking-widest 
            ${users.length < 2 ? 'text-neutral-500' : 'dark:text-neutral-400 text-stone-600 group-hover:text-neutral-300 transition-all duration-200'}`}>
            Start Game!
        </span>
    </button>
)}
