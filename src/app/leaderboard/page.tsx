          ...
          <div className="w-full flex flex-col">
            <div className={`w-full border border-stone-300 dark:border-neutral-600 rounded-xl sm:rounded-3xl py-1 px-2 sm:px-4 bg-white dark:bg-[#242120]`}>
              ...
              {getModeResults(selectedMode)?.topResults?.length ? (
                getModeResults(selectedMode)?.topResults.map((result, idx) => (
                  <div
                    key={idx}
                    className={`grid grid-cols-5 text-sm sm:text-base py-2 border-b last:border-b-0 border-stone-200 dark:border-neutral-700 text-center text-stone-400 ${
                      idx % 2 === 0 ? "bg-neutral-200 dark:bg-stone-800" : "bg-white dark:bg-[#242120]"
                    }`}
                  >
                    <span className="flex justify-center items-center">
                      {idx < 3 ? (
                        <PiMedalFill className={`${
                          idx === 0 ? "text-yellow-400" : 
                          idx === 1 ? "text-gray-300" : "text-[#CD7F32]"
                        } w-5 h-5`} />
                      ) : (
                        idx + 1
                      )}
                    </span>
                    <span className="px-1 truncate" title={result.playerName}>
                      {result.playerName}
                    </span>
                    <span className="px-1 truncate" title={result.subType}>
                      {result.subType}
                    </span>
                    <span className="px-1">{result.rawSpeed}</span>
